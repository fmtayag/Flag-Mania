...
class Text:
    def __init__(self, text, font, size, pos_y, color = WHITE):
        self.text = text
...
score = 0
question_number = 1
correct_display_tick = 0
### INPUT MAXIMUM NUMBER OF QUESTIONS HERE >>>>>>>>>>
max_questions = 30

FPS = 60
clock = pygame.time.Clock()
correct_answer = ["hey now"]
...
def game_loop():
    ...
    ## Randomizes flag selection
    rng = random.randint(0, len(choices) - 1)
    
    running = True
    
    while running == True:
        if correct_display_tick > 0: 
            correct_display_tick -= 1
        else:
            pass
        clock.tick(FPS)
        mouse_pressed = False
        ...
        correct_label = Text(f"Correct Answer is {correct_answer[0]}", "Pixelart.ttf", 16, screen_height - 30, GREEN)
        ...
        for i in range(3):
            if mouse_pressed == True and choices_list[i].label_rect.collidepoint(mouse_pos) and choices_list[i].text == choices[rng].name:
                ...
                if score == 0:
                    question_number += 1
                    correct_display_tick = 200
                    del correct_answer[0]
                    correct_answer.append(choices[rng].name)
                    game_loop()
                elif score > 0:
                    score -= 1
                    question_number += 1
        ...
        if correct_display_tick > 0:
            screen.blit(correct_label.label, correct_label.label_rect)
